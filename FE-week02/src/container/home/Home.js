...
import "./Home.scss";
import { getAllProductbystatus } from "../../services/productService";
...
const Home = () => {
  ...
  let getApi = async () => {
    let check = await getAllProductbystatus();
    if (check) {
      setAPI(check.data);
    }
  };
    ...
    <div className="container">
      ...
      <div>
        <div className="cart-icon">
          <div
            className="icon-cart w-1 h-1 p-1 rounded"
            onClick={() => handleCart()}
          >
            <FaCartArrowDown className="" />
            <span className="text-sl">
              {arrProduct && arrProduct.length > 0 ? arrProduct.length : 0}
            </span>
          </div>
        </div>
            ...
            return (
              <div key={index} className="item-product">
                ...
                <div className="price-item">
                  <span className="text-price">{item.price[0].price}</span>
                </div>
                <button
                  className="btn-item"
